...
module.exports = function (conn) {
    return new Object({
            ...
            addManufacturer: function (data, main_callback){
                let me = this;
                let saveTask = [
                    function (callback) {
                    ...
                    function (callback) {
                        conn.query("INSERT INTO `manufacturers` SET ?",[data.row] ,function (error, results, fields) {
                            if (error) {
                                console.log(error);
                            }else{
                                callback(null);
            ...
            updateManufacturer: function (data, main_callback){
                let me = this;
                let saveTask = [
                    function (callback) {
                    ...
                    function (callback) {
                        conn.query("UPDATE `manufacturers` SET ? WHERE `id`=?",[data.row,data.id] ,function (error, results, fields) {
                            if (error) {
                                console.log(error);
                            }else{
                                callback(null);
			...
			getManufacturers: function (callback){
				conn.query("SELECT * FROM `manufacturers`", function (error, results, fields) {
					if (error) {
						console.log(error);
					}else{
						callback(results);
